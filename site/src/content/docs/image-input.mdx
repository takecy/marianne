...
description: Four ways to bring an image into Marianne — clipboard paste, drag-and-drop, macOS "Open With Marianne" from Finder, and File → Open... (Cmd+O) from the menu bar.
---

import { Aside } from "@astrojs/starlight/components";

Images enter the canvas through four paths — pick whichever fits your workflow. The first three follow Skitch-style frictionless input (no dialog box); the fourth is the macOS-native **File → Open...** menu for users who prefer the conventional path.
...
flowchart LR
  ...
  O[File menu: Open... <br/> Cmd + O] -->|native file dialog| M
```
...
Press **`Cmd + O`** or choose **File → Open...** from the menu bar. A native file dialog opens, filtered to **PNG / JPEG**. The selected image is loaded through the same pipeline as "Open With Marianne".

This route runs entirely Rust-side: the dialog is opened by the `pick_and_open_image` command (not the frontend `@tauri-apps/plugin-dialog#open()` API), so the chosen file's read scope is only granted after the path has been validated against the symlink-rejection + extension whitelist trust boundary. There is no way for a malicious symlink or wrong-extension file to leak read access from this route.

The status bar shows the full file path on the left, same as the other path-based routes.
