...
- **4 種類のアノテーションツール:** 矩形 / 矢印 / テキスト / モザイク（機密情報の塗りつぶし向け）
- **8 色プリセット:** red / orange / blue / green / yellow / pink / black / white — 矩形・矢印・テキストに適用される
- **矩形は 4 段階の太さプリセット:** thin (6) / medium (12) / thick (18) / extraThick (28)。モザイクのブロックサイズは固定値
- **3 経路の画像入力:** クリップボードからのペースト（`Cmd/Ctrl + V`）、ウィンドウへのドラッグ&ドロップ、macOS Finder からの「このアプリケーションで開く」（アプリ未起動時 / 起動中の両方で動作）
- **2 系統のエクスポート:** PNG をファイル保存（ネイティブ保存ダイアログ）または PNG をシステムクリップボードへコピー
- **Option+drag で複製:** 選択中シェイプを Option (Alt) を押しながらドラッグするとドロップ位置に複製を生成する。元シェイプはその場に残り、新しい複製が選択状態になる
- **アプリ内シェイプクリップボード:** `Cmd/Ctrl + C` / `Cmd/Ctrl + V` でアプリ内のシェイプを 1 つだけコピー&ペースト。アプリ内クリップボードが空のときは OS の画像ペーストにフォールバックする
- **Undo / Redo:** セッションあたり最大 50 ステップ。画像を切り替えると履歴はリセットされる
- **ステータスバー:** 画像の取り込み元（ファイルパス / ドラッグ&ドロップ / ペースト）と自然サイズを表示
- **自動アップデート:** 起動時に新しいリリースをチェックし、ワンクリックで適用できる
- **完全オフライン動作:** ネットワーク通信は updater が GitHub の `releases/latest/download/latest.json` を取得する 1 回のみ。画像データが端末外へ出ることは一切ない

## キーボードショートカット

`Cmd` は macOS、他プラットフォームでは `Ctrl` に読み替える。テキストアノテーションの編集オーバーレイ表示中、およびネイティブ `input` / `textarea` にフォーカスがある間は全ショートカットが無効化される（ブラウザ標準の編集キーを尊重するため）。

| ショートカット              | 動作                                                | 補足                                                         |
| --------------------------- | --------------------------------------------------- | ------------------------------------------------------------ |
| `Cmd + Shift + S`           | PNG をファイル保存                                  | ネイティブの保存ダイアログを開く                             |
| `Cmd + Shift + C`           | PNG をシステムクリップボードへコピー                | `Cmd + C` とは別経路                                         |
| `Cmd + Z`                   | undo                                                | 最大 50 ステップ                                             |
| `Cmd + Shift + Z`           | redo                                                |                                                              |
| `Cmd + C`                   | 選択中シェイプをアプリ内クリップボードへコピー      | 選択モードでのみ動作                                         |
| `Cmd + V`                   | アプリ内クリップボードのシェイプを貼付              | クリップボードが空のときは OS の画像ペーストにフォールバック |
| `v` / `a` / `r` / `t` / `m` | ツール切替（select / arrow / rect / text / mosaic） | 画像未読み込み時は無視                                       |
| `Delete` / `Backspace`      | 選択中シェイプを削除                                | 選択モードでのみ動作                                         |
