...
- **Linter は oxlint**（`.oxlintrc.json`）。ESLint 系は撤去済み（issue #108）。壊しやすい不変条件が多いので変更前に必読:
  ...
  - 旧構成のうち oxlint で再現できないのは `no-octal` 1 件だけ（oxlint に存在しないが TS strict が legacy octal を拒否するため無害）。旧 `react-hooks/{refs, purity, immutability, set-state-in-effect, preserve-manual-memoization, ...}` の 14 個は、oxlint 1.79 が旧 `react/react-compiler` を分割した **`react/{refs, purity, immutability, set-state-in-render, set-state-in-effect, preserve-manual-memoization, use-memo, void-use-memo, static-components, globals, error-boundaries, incompatible-library}` の 12 ルール**で 1 対 1 にカバーされている。これらは `correctness` 経由でも入るが、上流のカテゴリ変更で静かに落ちないよう `rules` に明示列挙してある。
  ...
  - **React 検査の抑制は必要な 1 ルールだけを行単位で。** oxlint 1.79 で旧統合ルールが分割されたため、`react/refs` を抑制しても同じ範囲の `set-state-in-render` / `purity` / `immutability` は生きたまま残る（1.79 未満では統合ルールのブロック抑制が 14 検査すべてを黙らせる退行を招いた）。`CanvasArea.tsx` の render 中 ref 比較は `react/refs` を **行単位** で 3 箇所抑制しており、直下の `setDraft` / `setTextInput` / `setEditingTextId` / `setCropRect` は検査対象のまま残す必要がある。ルール名を広げたりブロック形式に戻すとこの分離が崩れ、その退行は設定回帰テストでも `--report-unused-disable-directives` でも検出できない。
